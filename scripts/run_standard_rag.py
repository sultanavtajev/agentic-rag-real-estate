"""CLI for standard RAG-analyse av eiendomsdokumenter."""
...
app = typer.Typer(help="Kjor standard RAG-analyse pa eiendomsdokumenter.")
console = Console()
...
def _display_results(result: AnalysisResult) -> None:
    ...
    Args:
        result: Analyseresultat fra pipeline.
    """
    console.print(f"\n[bold green]Analyse ferdig[/] — run_id: {result.run_id}")

    table = Table(title="Identifiserte risikoer")
...
def main(
    ...
    pipeline = StandardRAGPipeline(settings, vector_store)

    result = pipeline.analyze(document_set_id, query)
